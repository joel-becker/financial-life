import numpy as np
import pytest

from models.personal_finance import PersonalFinanceModel
from tests.test_tax_flow import make_params


def test_minimum_consumption_is_constant_in_real_terms():
    # The model runs in real terms, so the consumption floor must not be
    # deflated again — it applies at full value in every year
    model = PersonalFinanceModel(make_params(minimum_consumption=25000))
    model.inflation = np.full((1, 3), 0.03)
    model.total_wealth[:, 2] = 0
    model.cash[:, 2] = 100000
    model.market[:, 2] = 0
    model.retirement_account[:, 2] = 0
    consumption = model.calculate_consumption_amount(
        2, np.array([1000.0]), is_retired=np.array([False]), years_left=30
    )
    assert consumption[0] == pytest.approx(25000.0)
